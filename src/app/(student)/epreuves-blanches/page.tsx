...
import { ArrowRight, CheckCircle2, Clock, PencilRuler } from 'lucide-react';
...
export default async function StudentExamsPage() {
  ...
  const { data: examsRaw } = await supabase
    ...
    .select('id, title, college_id, duration_minutes, instructions, min_offer, target_colleges, voies, target_promos, publish_at, exam_mode, qroc_mode') as any;
                ...
                <li key={e.id}>
                  <Link href={`/epreuves-blanches/${e.id}`} className="group flex h-full flex-col rounded-2xl border border-(--color-border) bg-(--color-surface) p-4 shadow-(--shadow-soft) transition-all hover:-translate-y-0.5 hover:shadow-(--shadow-lifted)">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {done ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-[#E7F6EC] px-2 py-0.5 text-[11px] font-bold text-[#16793C]"><CheckCircle2 className="h-3 w-3" /> {sub!.percentage}%</span>
                      ) : e.duration_minutes ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-(--color-sand-100) px-2 py-0.5 text-[11px] font-semibold text-(--color-ink-soft)"><Clock className="h-3 w-3" /> {e.duration_minutes} min</span>
                      ) : null}
                    </div>
                    <p className="mt-3 flex-1 font-semibold text-(--color-ink)">{e.title}</p>
                    <p className="mt-1 text-xs text-(--color-ink-muted)">{e.college_id ? (collegeName.get(e.college_id) ?? '') : 'Transversale'}</p>
                    <span className="mt-3 inline-flex items-center gap-1 text-[13px] font-bold text-(--color-primary)">
                      {done ? 'Voir mes résultats' : 'Composer'} <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
                    </span>
                  </Link>
                </li>
              );
