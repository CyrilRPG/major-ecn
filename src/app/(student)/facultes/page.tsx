...
export default async function CollegesPage() {
  ...
  const colleges = (
    ((data as unknown as { semestres?: { matieres?: {
        ...
        {colleges.map((m) => {
          ...
          return (
            <Link
              key={m.id}
              href={`/matieres/${m.id}`}
              className="group flex flex-col justify-between rounded-2xl border border-(--color-border) bg-(--color-surface) p-5 transition-colors hover:border-(--color-accent) focus-ring"
            >
