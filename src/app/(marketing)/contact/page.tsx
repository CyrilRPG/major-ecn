    ...
    <>
      ...
      <section className="hero-navy relative isolate overflow-hidden text-white">
        ...
        <div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-20">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-(--color-accent)">Nous contacter</p>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24">
        <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
          <div>
            ...
            <h2 className="mt-3 font-display text-2xl tracking-tight text-(--color-ink)">
              Joignez Major ECN directement
            </h2>
            <ul className="mt-6 space-y-3">
              {COORD.map((c) => (
                <li
                  key={c.t}
                  className="card-lift flex items-center gap-4 rounded-2xl border border-(--color-border) bg-(--color-surface) p-4 shadow-(--shadow-soft) hover:shadow-(--shadow-lifted)"
                >
          ...
          <div className="rounded-3xl border border-(--color-border) bg-(--color-surface-soft) p-7 lg:p-9">
            ...
            <form action={`mailto:${CONTACT}`} method="post" encType="text/plain" className="mt-6 space-y-4">
              ...
              <button
                type="submit"
                className="group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-(--color-primary) px-6 py-3.5 text-base font-semibold text-white transition-transform hover:scale-[1.02]"
              >
                Envoyer ma demande
