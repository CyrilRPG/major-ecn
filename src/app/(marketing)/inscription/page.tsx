    ...
    <>
      <section className="bg-(--color-sidebar) text-white">
        <div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-20">
          ...
          <h1 className="mt-5 max-w-3xl font-display text-4xl leading-[1.08] tracking-tight sm:text-5xl">
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr]">
          <div>
            <h2 className="font-display text-3xl tracking-tight text-(--color-ink)">Comment s’inscrire</h2>
            <ol className="mt-6 space-y-4">
              {ETAPES.map((e, i) => (
                <li key={e.t} className="flex items-start gap-4 rounded-2xl border border-(--color-border) bg-(--color-surface) p-5 shadow-(--shadow-soft)">
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-(--color-primary) text-white">
                    <e.Icon className="h-5 w-5" />
                  </span>
                  <div>
                    <p className="text-sm font-semibold text-(--color-ink)">
                      <span className="mr-1.5 text-(--color-ink-muted)">{i + 1}.</span>{e.t}
                    </p>
            ...
            <div className="mt-6 rounded-2xl border border-(--color-border) bg-(--color-surface-soft) p-5">
          ...
          {/* Formulaire */}
          <div className="rounded-3xl border-2 border-(--color-primary) bg-(--color-surface) p-7 shadow-(--shadow-lifted) lg:p-9">
            <h2 className="font-display text-2xl text-(--color-ink)">Demande d’inscription</h2>
            <p className="mt-1.5 text-sm text-(--color-ink-soft)">
              Renseignez vos informations, nous vous recontactons sous 24 h.
            </p>
            <form action={`mailto:${CONTACT}`} method="post" encType="text/plain" className="mt-6 space-y-4">
              ...
              <button
                type="submit"
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-(--color-primary) px-6 py-4 text-base font-semibold text-white transition-transform hover:scale-[1.02]"
              >
