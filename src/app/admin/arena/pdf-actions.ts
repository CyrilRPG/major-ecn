...
export async function prepareCorrectionsUpload(
  roundId: string,
): Promise<Ok<{ path: string; token: string }> | Err> {
  try {
    ...
    const { data, error } = await arenaDb()
      .storage.from(ARENA_BUCKET)
      .createSignedUploadUrl(path);
    if (error || !data)
      return {
        ok: false,
        error: error?.message ?? "URL d’envoi indisponible.",
      };
...
export async function attachCorrectionsPdf(
  roundId: string,
  path: string,
): Promise<Ok | Err> {
  try {
    ...
    const round = await getRound(roundId);
    if (!round) return { ok: false, error: "Manche introuvable." };
    if (!path.startsWith(`${round.tournament_id}/`))
      return { ok: false, error: "Chemin invalide." };
    const { data: exists, error: storageError } = await arenaDb()
      .storage.from(ARENA_BUCKET).exists(path);
    if (storageError || !exists)
      return { ok: false, error: "Le PDF n’a pas été reçu. Réessayez l’envoi du fichier." };
    await arenaDb()
      .from("arena_rounds")
      .update({
    ...
    await logAdmin(actor, {
      ...
      roundId,
      kind: "pdf_uploaded",
      details: `PDF de corrections fourni par Major ECN (${path}).`,
    });
...
export async function removeCorrectionsPdf(roundId: string): Promise<Ok | Err> {
  try {
    ...
    await logAdmin(actor, {
      ...
      roundId,
      kind: "pdf_removed",
      details: "PDF de corrections retiré.",
    });
    revalidatePath(`/admin/arena/${round.tournament_id}`);
    return { ok: true };
  } catch (error) {
