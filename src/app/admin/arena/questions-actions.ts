...
export async function importFromBank(
  roundId: string,
  questionIds: string[],
): Promise<Ok<{ inserted: number }> | Err> {
  try {
    const actor = await ensureArenaAdmin();
    const round = await getRound(roundId);
    if (!round) return err("Manche introuvable.");
    ...
    for (const q of (data ?? []) as any[]) {
      ...
      rows.push({
        ...
        images: Array.isArray(q.images)
          ...
          : [],
        items: clean,
        explanation: String(q.correction_generale ?? "")
          .replace(/<[^>]*>/g, " ")
          .trim(),
    ...
    await logAdmin(actor, {
      ...
      newValue: { inserted: rows.length },
      details: `${rows.length} question(s) copiée(s) depuis la banque pour la manche ${round.number}.`,
    });
    revalidate(round.tournament_id);
