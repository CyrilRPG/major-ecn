/**
 * Maintien des acquis (section 5) et spécialités étudiées — module PARTAGÉ.
 *
 * Aucune dépendance au serveur : toutes les fonctions reçoivent le client
 * Supabase en argument. L'app mobile en embarque une copie octet-identique
 * (`application/mobile/src/shared/lib/pedago/maintien-core.ts`) pour que la
 * sélection des questions et le compteur de révisions soient EXACTEMENT ceux
 * de la plateforme web. Le web importe `maintien.ts` (même API, gardée
 * `server-only`).
 */
import type { SupabaseClient } from '@supabase/supabase-js';
import { EDN_FACULTE_ID } from '../data/faculte';
import { fetchAllRows } from '../supabase/fetch-all-pure';
import { coursAnnalesParMatiere } from '../data/annales';
import { canAccessCollege, type parseScope } from '../auth/permissions';
import {
