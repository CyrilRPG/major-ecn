...
4. QCM : conserve les lettres source (A à K), chaque proposition avec son vrai/faux et sa justification. QROC : réponse attendue (variantes séparées par |) et corrigé s'ils figurent dans la source. Une liste de propositions COUPÉE PAR UN SAUT DE PAGE se poursuit en haut de la page suivante (« c) … d) … e) … » sans rappel de la question) : va la chercher et restitue la liste ENTIÈRE. Une question rendue avec deux propositions alors que la suivante commence par c) est une extraction incomplète.
