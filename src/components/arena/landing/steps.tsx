...
import { BODY, HEADLINE, LIGHT } from '../tokens';
...
export function LandingSteps({ questions, minutes }: { questions: number; minutes: number }) {
  ...
  return (
    <section style={{ background: LIGHT.bg, color: LIGHT.text }}>
      <Container className="py-12 sm:py-16">
        <div className="grid gap-10 md:grid-cols-3 md:gap-0">
          {steps.map((s, i) => (
            <div key={s.n} className={`flex flex-col items-center text-center md:px-8 ${i > 0 ? 'md:border-l' : ''}`} style={{ borderColor: LIGHT.lineStrong }}>
              <div className="flex items-end gap-4">
                <span className="text-[4.6rem] leading-none sm:text-[5.4rem]" style={{ fontFamily: HEADLINE, color: '#B8892E', letterSpacing: '0.02em' }}>{s.n}</span>
