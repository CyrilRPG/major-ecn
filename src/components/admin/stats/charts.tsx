        ...
        <AreaChart data={data}>
          <defs>
            <linearGradient id="grad-activity" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#1C0328" stopOpacity={0.45} />
              <stop offset="100%" stopColor="#1C0328" stopOpacity={0} />
            </linearGradient>
          </defs>
          ...
          />
          <Area type="monotone" dataKey="value" stroke="#1C0328" strokeWidth={2} fill="url(#grad-activity)" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data}>
          ...
          <Tooltip
            ...
            contentStyle={{ borderRadius: 12, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}
          />
          <Bar dataKey="value" fill="#1C0328" radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical">
          ...
          <Tooltip
            ...
            contentStyle={{ borderRadius: 12, border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}
          />
          <Bar dataKey="value" fill="#F4AB34" radius={[0, 8, 8, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
