-- Publie en une transaction le contenu généré d'un chapitre.
-- Toute erreur annule également les suppressions de l'ancienne version : il ne
-- peut donc plus exister de série avec des questions sans items après coupure.
...
begin
  ...
  if p_replace then
    ...
    -- Les annales (type='annale') ne sont jamais effacées par un import généré.
  ...
  loop
    ...
    loop
      ...
      if coalesce(v_series_entry->>'label', '') ~* '^DP[[:space:]]+[0-9]'
         ...
         and coalesce(v_question_entry->>'enonce', '') !~* 'nouvel élément' then
        raise exception 'DP « % », question % sans Nouvel élément', v_series_entry->>'label', v_question_ordinal;
      end if;

      insert into public.qcm_questions (serie_id, enonce, order_index, format, reponse_attendue, correction_generale)
      values (v_serie_id, v_question_entry->>'enonce', v_question_ordinal, 'qcm', null, nullif(v_question_entry->>'correction_generale', ''))
