-- T7 : get_accueil_stats — qcm_counts inclut n_qroc + fc_counts par cours
-- (fallback voie-aware si le cache faculté get_faculte_content_totals n'est pas
-- disponible). Permet à l'accueil de calculer « Total étudié » sur le seul
-- contenu réellement accessible (voie externe = QROC, interne = QCM/DP ; et
-- flashcards limitées aux collèges accessibles).
