...
--
-- Tout élève inscrit en Gériatrie (col-geriatrie), quelle que soit sa
-- formule, reçoit en plus un accès à Médecine générale — restreint aux
-- items du dossier « PLATEFORME GERIATRIE _ ITEMS A RAJOUTER » (liste tenue
-- à jour dans src/lib/auth/geriatrie-mg-bonus.ts, GERIATRIE_MG_BONUS_COURS_IDS).
-- Sur ces cours, aucun QCM/DP de Médecine générale ne doit être visible,
-- seuls les QROC restent accessibles — indépendamment de la voie de
-- l'élève (qui reste inchangée pour son propre accès Gériatrie).

-- 1) Backfill : élèves déjà inscrits en Gériatrie.
do $$
declare
...
begin
  ...
  loop
    select coalesce(array_agg(id::text), array[]::text[]) into geriatrie_cours_ids
      from public.cours where matiere_id = 'col-geriatrie';

    select coalesce(array_agg(distinct v), array[]::text[]) into merged_cours
      from unnest(
        coalesce((select array_agg(x) from jsonb_array_elements_text(coalesce(r.permission_scope->'cours', '[]'::jsonb)) x), array[]::text[])
        || geriatrie_cours_ids
        || bonus_cours_ids
      ) v;

    select case when r.permission_scope->'colleges' @> '"col-medecine-generale"'::jsonb
      then r.permission_scope->'colleges'
      else (r.permission_scope->'colleges') || '"col-medecine-generale"'::jsonb
    end into merged_colleges;

    update public.profiles
    set permission_scope = jsonb_set(
      jsonb_set(r.permission_scope, '{colleges}', merged_colleges, true),
...
-- 2) RLS : bloque les QCM/DP de Médecine générale pour les élèves inscrits en
-- Gériatrie (seuls les QROC restent visibles), en plus de la restriction de
-- voie existante (qcm_series_voie_restrict). Policy restrictive : combinée en
-- AND avec les autres policies restrictives déjà en place.
create or replace function public.current_is_geriatrie() returns boolean
  language sql stable security definer set search_path to 'public' as $$
  select coalesce(
    (select permission_scope->'colleges' @> '"col-geriatrie"'::jsonb from public.profiles where id = auth.uid()),
    false
  );
$$;

drop policy if exists qcm_series_geriatrie_mg_qroc_only on public.qcm_series;
create policy qcm_series_geriatrie_mg_qroc_only
  ...
  using (
    ...
    or coalesce(qcm_series.kind, 'qcm') = 'qroc'
  );
