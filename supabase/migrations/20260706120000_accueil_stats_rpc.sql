-- P1 optimisation accueil : agrège tout le tableau de bord en UN appel RPC.
-- Remplace 6 requêtes PostgREST à jointures profondes (qui remontaient des
-- milliers de lignes vers le composant serveur) + 1 requête séquentielle
-- (qcmAccessibleTotal). Tout le calcul lourd est poussé dans Postgres.
...
att as (
  select a.is_correct, a.attempted_at, s.cours_id
  from qcm_attempts a
...
select jsonb_build_object(
  ...
  'per_cours', coalesce((select jsonb_agg(jsonb_build_object(
      'cours_id', cours_id, 'attempts', attempts, 'correct', correct, 'has_fc', has_fc)) from per_cours), '[]'::jsonb),
