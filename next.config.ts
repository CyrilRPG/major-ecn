...
const nextConfig: NextConfig = {
  ...
  outputFileTracingIncludes: {
    ...
    // Médaillons composés : la route embarque le portrait en base 64 dans le
    // SVG (un SVG chargé par une balise <img> ne va chercher aucune image
    // externe). Elle lit donc les PNG sur disque, alors que `public/**` est
    // exclu du trace pour toutes les fonctions (voir plus bas) — sans cette
    // réinclusion, la route renvoie 500 en production (constaté le 21/09/2026,
    // déploiement d600cdce). 24 fichiers, 784 Ko.
    '/api/avatar/[seed]': ['./public/arena/avatars/*.png'],
